var n, p; // empty variables.
var points = 25;  // overall number of matches.
...
function add(n){ // function for generating events of the button.
   	if (points >= n){ // if the overall number of matches is bigger than the number of matches that I take now
   		points = doTheOperation(n, points, myPoints, "text1", "myMatches");
   		myPoints += n;
   		compTake = Math.floor(Math.random()*3)+1;
  		if (compTake <= points){
